...
class EMNISTModel(hk.Module):
    ...
    """
    def __init__(self, num_classes, act_fn):
        super().__init__()
...
class CIFAR10Model(hk.Module):
    ...
    Model for CIFAR-10
    """
    def __init__(self, num_classes, act_fn, seed=42):
        super().__init__()
        self._num_classes = num_classes
        self._act_fn = act_fn
        self._seed = seed
    ...
    def forward_pass(batch, is_train=True):
        if custom_model is None:
            if dataset_name == "mnist":
                return MNISTModel(num_classes, act_fn=params["act_fn"])(batch['x'])
            elif dataset_name == "emnist" or dataset_name == "svhn_cropped":
                return EMNISTModel(num_classes, act_fn=params["act_fn"])(batch['x'])
            elif dataset_name == "cifar10":
                return CIFAR10Model(num_classes, act_fn=params["act_fn"], seed=seed)(
                    batch['x'], is_train)
            else:
